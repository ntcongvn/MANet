...
def make_one_rcnn_target(cfg, input, proposal, truth_box, truth_label):
    ...
    if len(truth_box) == 0:
        num_bg = min(len(proposal), cfg['rcnn_train_batch_size'])                 #64
    ...
    fg_index = np.where(max_overlap >= cfg['rcnn_train_fg_thresh_low'])[0]      #0.5
    bg_index = np.where(max_overlap <  cfg['rcnn_train_bg_thresh_high'])[0]     #0.1

    # sampling for class balance
    num_class = cfg['num_class']                                                #2
    num = cfg['rcnn_train_batch_size']                                          #64
    num_fg = int(np.round(cfg['rcnn_train_fg_fraction'] * cfg['rcnn_train_batch_size']))          # 0.5     64
