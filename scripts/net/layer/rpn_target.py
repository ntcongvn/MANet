...
def make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label):
    ...
    if num_truth_box:
        ...
        # a threshold
        bg_index = np.where(max_overlap < cfg['rpn_train_bg_thresh_high'])[0]       #0.02
        label[bg_index] = 0
        label_weight[bg_index] = 1

        # The anchor box is a sure foreground, if its largest IoU is larger or 
        # equal than a threshold
        fg_index = np.where(max_overlap >= cfg['rpn_train_fg_thresh_low'])[0]       #0.5
        label[fg_index] = 1
        ...
        #idx = random.sample(range(len(fg_index)), min(len(fg_index), 2))
        #idx = random.sample(range(len(fg_index)), min(len(fg_index), len(truth_box)))
        #print("fg_index:",len(fg_index))
        #print("truth_box:",len(truth_box))
        #print("idx:",len(idx))
...
def make_rpn_target(cfg, mode, inputs, window, truth_boxes, truth_labels):
    ...
    fg_index = np.where( (rpn_label_weights.cpu()!=0) & (rpn_labels.cpu()!=0))[0]
    bg_index = np.where( (rpn_label_weights.cpu()!=0) & (rpn_labels.cpu()==0))[0]
