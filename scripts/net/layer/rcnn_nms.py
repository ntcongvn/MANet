...
def rcnn_nms(cfg, mode, inputs, proposals, logits, deltas):
    ...
    if mode in ['train',]:
        nms_pre_score_threshold = cfg['rcnn_train_nms_pre_score_threshold']       #0.5
        nms_overlap_threshold   = cfg['rcnn_train_nms_overlap_threshold']         #0.1

    elif mode in ['valid', 'test','eval']:
        nms_pre_score_threshold = cfg['rcnn_test_nms_pre_score_threshold']        #0.0
        nms_overlap_threshold   = cfg['rcnn_test_nms_overlap_threshold']          #0.1
        #
        # if mode in ['eval']:
...
def rcnn_nms(cfg, mode, inputs, proposals, logits, deltas):
    ...
    for b in range(batch_size):
        ...
        if len(index)>0:
            ...
            for j in range(1, num_class): #skip background
                score=(proposal[:,1:2]+prob[:, j].reshape(-1, 1))/2
                idx = np.where(score > nms_pre_score_threshold)[0]
                # idx = np.where(cats == j)[0]
                if len(idx)>0:
                    p=score[idx]
                    #p = prob[idx, j].reshape(-1, 1)
                    #p= (prob[idx,1:2]+prob2[idx, j].reshape(-1, 1))/2
                    d = delta[idx, j]
                    # m = mask[idx, j - 1]
                    box = rcnn_decode(proposal[idx, 2:8], d, cfg['box_reg_weight'])
                    box = clip_boxes(box, inputs.shape[2:])
