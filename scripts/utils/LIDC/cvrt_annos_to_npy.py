...
from dataset.dataset_mapping import get_convertpath
import gc
...
def arrs2mask(img_dir, ctr_arr_dir, save_dir):
    ...
    path_convert=get_convertpath(img_dir)
    pids = list(path_convert.keys())
    for pid in  tqdm(pids, total=len(pids)):
        print(os.path.join(img_dir, '%s.mhd' % (path_convert[pid])))
        img, origin, spacing = load_itk_image(os.path.join(img_dir, '%s.mhd' % (path_convert[pid])))
...
#             nrrd.write(os.path.join(save_dir, str(cons), pid), mask)
        del nodule_masks, same_nodules,masks
        del img, origin, spacing,ctr_arrs
        gc.collect()
    print(consensus)
    print(cnt)
