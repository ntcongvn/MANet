...
def augment(sample, target, masks, do_flip = True, do_rotate=True, do_swap = True):
    masks = (masks > 0).astype(np.int32)
    if do_rotate:
        plane=np.random.choice(4,1)[0]
        if plane==0: #yz
          validrot = False
          counter = 0
          while not validrot:
              newtarget = np.copy(target)
              angle1 = (np.random.rand()-0.5)*60
              size = np.array(sample.shape[1:3]).astype('float')
              rotmat = np.array([[np.cos(angle1/180*np.pi),-np.sin(angle1/180*np.pi)],[np.sin(angle1/180*np.pi),np.cos(angle1/180*np.pi)]])
              newtarget[0:2] = np.dot(rotmat,target[0:2]-size/2)+size/2
              if np.all(newtarget[:3]>target[3]) and np.all(newtarget[:3]< np.array(sample.shape[1:4])-newtarget[3]):
                  #print("yz")
                  validrot = True
                  target = newtarget
                  sample = rotate(sample,angle1,axes=(1,2),reshape=False)
                  masks = rotate(masks, angle1, axes=(0,1), reshape=False)
              else:
                  counter += 1
                  if counter ==3:
                      break
          ...
          while not validrot:
              newtarget = np.copy(target)
              angle1 = (np.random.rand()-0.5)*60
              size = np.array([sample.shape[1],sample.shape[3]]).astype('float')
              rotmat = np.array([[np.cos(angle1/180*np.pi),-np.sin(angle1/180*np.pi)],[np.sin(angle1/180*np.pi),np.cos(angle1/180*np.pi)]])
              newcomput = np.dot(rotmat,np.array([target[0],target[2]])-size/2)+size/2
              newtarget[0]=newcomput[0]
              newtarget[2] = newcomput[1]
              if np.all(newtarget[:3]>target[3]) and np.all(newtarget[:3]< np.array(sample.shape[1:4])-newtarget[3]):
                  #print("xz")
                  validrot = True
                  target = newtarget
                  sample = rotate(sample,angle1,axes=(1,3),reshape=False)
                  masks = rotate(masks, angle1, axes=(0,2), reshape=False)
              else:
                  counter += 1
                  if counter ==3:
                      break
