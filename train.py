...
parser.add_argument('--processbar', default=True, type=bool,
                    metavar='PB', help='show processbar of tqdm')
...
def main():
    ...
    for i in tqdm(range(start_epoch, epochs + 1), desc='Total',disable=processbar):
        ...
        train(net, train_loader, optimizer, i, train_writer,processbar)
        validate(net, val_loader, i, val_writer,processbar)
        print
...
def train(net, train_loader, optimizer, epoch, writer,processbar=True):
    net.set_mode('train')
    s = time.time()
    ...
    for j, (input, truth_box, truth_label, truth_mask, masks) in tqdm(enumerate(train_loader), total=len(train_loader), desc='Train %d' % epoch,disable=processbar):
        ...
        loss, rpn_stat, rcnn_stat, mask_stat,ds_loss = net.loss()
        optimizer.zero_grad()
        ...
        dss_loss.append(ds_loss.cpu().data.item())
    ...
    rpn_stats = torch.tensor(rpn_stats, device = 'cpu')
    rpn_stats = np.asarray(rpn_stats, np.float32)
    
    train_info1='Train Epoch %d| iter %d| total time %f| loss %f' % (epoch, j, time.time() - s, np.average(total_loss))
    train_info2='rpn_cls %f| rpn_reg %f| rcnn_cls %f| rcnn_reg %f| ds_loss %f| mask_loss %f' % \
        ( np.average(rpn_cls_loss), np.average(rpn_reg_loss),
          np.average(rcnn_cls_loss), np.average(rcnn_reg_loss),
          np.average(dss_loss),np.average(mask_loss))
    train_info3='rpn_stats: tpr %f| tnr %f| total pos %d| total neg %d| reg %.4f| %.4f| %.4f| %.4f| %.4f| %.4f' % (
    ...
    if net.use_rcnn:
        ...
        train_info4='rcnn_stats: reg %.4f, %.4f, %.4f, %.4f, %.4f, %.4f' % (
    ...
    if net.use_mask:
        ...
        for i in range(len(mask_stats[0])):
            ...
            train_info5=train_info5+config['roi_names'][i]+ ': '+ str(np.round(np.mean(s_region), 4))+', '
            writer.add_scalar('mask_%s' % (config['roi_names'][i]), np.round(np.mean(s_region), 4), epoch)

    print(train_info1,">>",train_info2,">>",train_info3,">>",train_info4,">>",train_info5)
    print
    print
    

def validate(net, val_loader, epoch, writer,processbar=True):
    net.set_mode('valid')
    rpn_cls_loss, rpn_reg_loss = [], []
    rcnn_cls_loss, rcnn_reg_loss = [], []
    mask_loss = []
    ...
    s = time.time()
    for j, (input, truth_box, truth_label, truth_mask, masks) in tqdm(enumerate(val_loader), total=len(val_loader), desc='Val %d' % epoch,disable=processbar):
        with torch.no_grad():
            input = Variable(input).cuda()
            truth_box = np.array(truth_box)
            truth_label = np.array(truth_label)
    ...
    rpn_stats = torch.tensor(rpn_stats, device = 'cpu')
    rpn_stats = np.asarray(rpn_stats, np.float32)
    val_info1='Val Epoch %d| iter %d| total time %f| loss %f' % (epoch, j, time.time()-s, np.average(total_loss))
    val_info2='rpn_cls %f| rpn_reg %f| rcnn_cls %f| rcnn_reg %f| ds_loss %f| mask_loss %f' % \
        (np.average(rpn_cls_loss), np.average(rpn_reg_loss),
            np.average(rcnn_cls_loss), np.average(rcnn_reg_loss),
            np.average(dss_loss),np.average(mask_loss))
    val_info3='rpn_stats: tpr %f| tnr %f| total pos %d| total neg %d| reg %.4f| %.4f| %.4f| %.4f| %.4f| %.4f' % (
        100.0 * np.sum(rpn_stats[:, 0]) / np.sum(rpn_stats[:, 1]),
        100.0 * np.sum(rpn_stats[:, 2]) / np.sum(rpn_stats[:, 3]),
        np.sum(rpn_stats[:, 1]),
        np.sum(rpn_stats[:, 3]),
    ...
    if net.use_rcnn:
        ...
        confusion_matrix = np.sum(confusion_matrix, 0)
        val_info4='rcnn_stats: reg %.4f, %.4f, %.4f, %.4f, %.4f, %.4f' % (
            np.mean(rcnn_stats[:, 0]),
            np.mean(rcnn_stats[:, 1]),
            np.mean(rcnn_stats[:, 2]),
            np.mean(rcnn_stats[:, 3]),
    ...
    if net.use_mask:
        ...
        for i in range(len(mask_stats[0])):
            ...
            s_region = s_region[~np.isnan(s_region)]
            #print(config['roi_names'][i], ': ', np.round(np.mean(s_region), 4), ', ',)
            val_info5=val_info5+config['roi_names'][i]+': '+ str(np.round(np.mean(s_region), 4))+', '
            writer.add_scalar('mask_%s' % (config['roi_names'][i]), np.round(np.mean(s_region), 4), epoch)
    print(val_info1,">>",val_info2,">>",val_info3,">>",val_info4,">>",val_info5)
    print
    print
